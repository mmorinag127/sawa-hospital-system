...
def _build_user_prompt(template: dict) -> str:
    custom = str(template.get("openai_ocr_user_prompt") or "").strip()
    if custom:
        return custom
    return "Read the attached fax image and return JSON following the system instruction."


def _extract_completion_text(response_payload: dict[str, Any]) -> str:
...
def _extract_json_payload_with_meta(text: str) -> tuple[dict[str, Any], bool]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("OpenAI OCR response is not JSON object")
...
def _recover_truncated_payload(raw: str) -> dict[str, Any] | None:
    ...
    facility_match = re.search(r'"facility_name"\s*:\s*("([^"\\]|\\.)*")', raw, re.S)
    if facility_match:
    ...
    for chunk in row_chunks:
        ...
        for key, value in row_obj.items():
            ...
            if not key_text:
                continue
            normalized_row[key_text] = str(value or "")
        if normalized_row:
...
def _is_truncated_finish_reason(finish_reason: str | None) -> bool:
    ...
    return token in {"LENGTH", "MAX_TOKENS", "MAX_OUTPUT_TOKENS", "MAX_OUTPUT_TOKEN"}
...
def _request_openai_json(
    *,
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI OCR timeout after {timeout:.0f}s") from exc
        ...
        try:
            ...
            detail = str(exc)
        raise RuntimeError(f"OpenAI OCR HTTP {exc.code}: {detail}") from exc
...
def run_openai_ocr(
    *,
    ...
    retry_on_truncation = _as_bool(
        ...
        if template.get("openai_ocr_retry_on_truncation") is not None
        else os.getenv("OPENAI_OCR_RETRY_ON_TRUNCATION"),
        default=True,
    )
    retry_max_tokens = _safe_int(
        template.get("openai_ocr_retry_max_tokens")
        or os.getenv("OPENAI_OCR_RETRY_MAX_TOKENS", "12000"),
        12000,
    )
    ...
    for attempt in range(1, 3):
        ...
        if not text:
            raise RuntimeError("OpenAI OCR returned empty response")
