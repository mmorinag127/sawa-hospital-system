...
def _ocr_quantity_cell(
    *,
    ...
    try:
        ...
        logger.warning("Hakodate cell OCR unavailable: {}", str(exc))
        raise HakodateOcrUnavailable(str(exc)) from exc

    image_width, image_height = page_image.size
    inner_margin = float(template.get("hakodate_cell_ocr_inner_margin_ratio") or 0.08)
    inner_margin = max(0.0, min(inner_margin, 0.3))
    crop_px = _normalized_bbox_to_px(
        fax_cell_bbox,
        image_width=image_width,
        image_height=image_height,
        inner_margin_ratio=inner_margin,
    )
    if not crop_px:
        return HakodateCellOcrResult(text="", normalized="", crop_bbox_px=None)
    crop = page_image.crop(tuple(crop_px))
    crop = ImageOps.autocontrast(crop.convert("L"))
    upscale = float(template.get("hakodate_cell_ocr_upscale") or template.get("hakodate_ocr_upscale") or 3.0)
    upscale = max(1.0, min(upscale, 5.0))
    if upscale > 1.0:
        crop = crop.resize((max(1, int(crop.size[0] * upscale)), max(1, int(crop.size[1] * upscale))))
    ...
    if debug_dir:
        try:
            ...
            out_dir = Path(debug_dir) / "cell_crops"
            ...
            logger.warning("Failed to save Hakodate cell crop: {}", str(exc))

    raw_configs = template.get("hakodate_cell_ocr_configs")
    if isinstance(raw_configs, list) and raw_configs:
        configs = [str(value) for value in raw_configs if str(value).strip()]
    else:
        configs = [
    ...
    for config in configs:
        try:
            text = str(pytesseract.image_to_string(crop, config=config) or "").strip()
        except Exception:
            continue
        ...
        if text and not best_text:
            best_text = text
    return HakodateCellOcrResult(
        text=best_text,
        normalized=_normalize_digits(best_text),
        confidence=None,
...
def _extract_tesseract_tokens(pdf_bytes: bytes, template: dict[str, Any]) -> list[HakodateToken]:
    try:
        ...
        logger.warning("Hakodate token OCR unavailable: {}", str(exc))
        raise HakodateOcrUnavailable(str(exc)) from exc

    page_index = max(int(template.get("page", 1)) - 1, 0)
    resolution = int(
        template.get("hakodate_ocr_resolution")
        or template.get("token_ocr_resolution")
        or template.get("grid_dpi")
        or 300
    )
    with pdfplumber.open(BytesIO(pdf_bytes)) as pdf:
    ...
    crop_box = template.get("hakodate_ocr_crop_box") or template.get("grid_table_box") or template.get("table_box")
    crop_x0, crop_y0, crop_x1, crop_y1 = 0, 0, page_width, page_height
    if isinstance(crop_box, list) and len(crop_box) >= 4:
        try:
            x0, y0, x1, y1 = [float(value) for value in crop_box[:4]]
            if max(x0, y0, x1, y1) <= 1.5:
                crop_x0 = max(0, min(page_width, int(x0 * page_width)))
                crop_y0 = max(0, min(page_height, int(y0 * page_height)))
                crop_x1 = max(0, min(page_width, int(x1 * page_width)))
                crop_y1 = max(0, min(page_height, int(y1 * page_height)))
            else:
                crop_x0 = max(0, min(page_width, int(x0)))
                crop_y0 = max(0, min(page_height, int(y0)))
                crop_x1 = max(0, min(page_width, int(x1)))
                crop_y1 = max(0, min(page_height, int(y1)))
        except Exception:
            crop_x0, crop_y0, crop_x1, crop_y1 = 0, 0, page_width, page_height
    if crop_x1 <= crop_x0 or crop_y1 <= crop_y0:
        crop_x0, crop_y0, crop_x1, crop_y1 = 0, 0, page_width, page_height

    image = page_image.crop((crop_x0, crop_y0, crop_x1, crop_y1))
    upscale = float(template.get("hakodate_ocr_upscale") or 2.0)
    upscale = max(1.0, min(upscale, 4.0))
    image = ImageOps.autocontrast(image.convert("L"))
    if upscale > 1.0:
        image = image.resize(
            (max(1, int(image.size[0] * upscale)), max(1, int(image.size[1] * upscale)))
        )
    raw_configs = template.get("hakodate_ocr_configs")
    if isinstance(raw_configs, list) and raw_configs:
        configs = [str(value) for value in raw_configs if str(value).strip()]
    else:
        configs = [
    ...
    for config in configs:
        try:
            ...
            logger.warning("Hakodate token OCR failed: {}", str(exc))
            raise HakodateOcrUnavailable(str(exc)) from exc
        for text, left, top, token_width, token_height, conf in zip(
            data.get("text", []),
            data.get("left", []),
            data.get("top", []),
            data.get("width", []),
            ...
            if not raw_text:
                continue
            try:
                left_unscaled = float(left) / upscale
                top_unscaled = float(top) / upscale
                width_unscaled = float(token_width) / upscale
                height_unscaled = float(token_height) / upscale
                page_x0 = crop_x0 + left_unscaled
                page_y0 = crop_y0 + top_unscaled
                page_x1 = page_x0 + width_unscaled
                page_y1 = page_y0 + height_unscaled
                x0 = max(0.0, min(1.0, page_x0 / page_width))
                y0 = max(0.0, min(1.0, page_y0 / page_height))
                x1 = max(0.0, min(1.0, page_x1 / page_width))
                y1 = max(0.0, min(1.0, page_y1 / page_height))
                confidence = float(conf)
                if confidence < 0:
                    confidence_value = None
                else:
                    confidence_value = confidence / 100.0 if confidence > 1 else confidence
            except Exception:
                continue
            key = (
                _normalize_digits(raw_text),
                round(x0 * 1000),
                round(y0 * 1000),
                round(x1 * 1000),
                round(y1 * 1000),
