...
resource "google_logging_metric" "watch_refresh_failures" {
  ...
  filter = <<-EOT
    ...
    ("Watch refresh skipped" OR "watch refresh failed" OR "Gmail watch refresh failed")
  EOT
  metric_descriptor {
    metric_kind = "DELTA"
    value_type  = "INT64"
...
resource "google_logging_metric" "gmail_invalid_grant" {
  name   = "gmail_invalid_grant_${var.env}"
  filter = <<-EOT
    resource.type="cloud_run_revision"
    resource.labels.service_name="${var.worker_service_name}"
    ("invalid_grant" OR "Gmail watch refresh failed")
  EOT
  metric_descriptor {
    metric_kind = "DELTA"
    value_type  = "INT64"
...
resource "google_monitoring_alert_policy" "gmail_invalid_grant" {
  display_name = "gmail-invalid-grant-${var.env}"
  combiner     = "OR"
  notification_channels = local.notification_channel_ids
  alert_strategy {
    auto_close = "86400s"
  ...
  conditions {
    display_name = "Gmail invalid_grant detected"
    condition_threshold {
      filter = "resource.type=\"cloud_run_revision\" resource.label.service_name=\"${var.worker_service_name}\" metric.type=\"logging.googleapis.com/user/${google_logging_metric.gmail_invalid_grant.name}\""
      comparison      = "COMPARISON_GT"
      threshold_value = 2
      duration        = "3600s"
      aggregations {
